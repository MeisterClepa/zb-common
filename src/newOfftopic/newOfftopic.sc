...
patterns:
    ...
    $drugs = (наркот*/марихуан*/гашиш*/транквилизатор*/амфетамин*/экстази/опиат*/барбитурат*/метамфетамин*/джоинт*/кокаин*/~героин/лсд/попперс*/амф*/спид*/кислот*/[запрещен*/наркотикосодержащие] веществ*/дурь*/крэк*/анаш*/индик*/сатив*/эфедр*)
    $maybeDrugs =  (скорост*/травк*/камен*/колес*/фен*/фед*/марк*/плюшк*/точк*/план*/гарик*/бошк*/шишк*/люс*/люськ*)
...
theme: /NewOfftopic
    ...
    state: HowAreYou
        ...
        q!: [привет] * как* * [$you|твои|твое|ваш*] * (дела|де ла|делишки|делаа|дила|себя (чувствуешь|чувствуете)|поживаешь|поживаете|настроение|жизнь) *
    ...
    state: NeedHelp
        ...
        q!: * {[~мочь] (~оказать/~оказывать/окажи/окажите) ~услуга} *
    ...
    state: HaveQuestion
        ...
        q!: * {(кое* */меня) вопрос*} *
        q!: * [один] вопрос* *
        q!: * (расскажи*/скажи*/просвети*) [ка] [мне/нам/меня/нас] [$AnyWord] $AnyWord (вещь/штуку/вещи/штуки) *
    ...
    state: WhatGenderAreYou
        ...
        q!: * $you ($man/$woman) *
        q!: * ($morph<Г дст 2л нст ед>/$morph<VERB>/$morph<V>) как * ($man/$woman) *
    ...
    state: WhatIsYourFavoriteColor
        ...
        q!: * [~какой/что] [у тебя/тебе/больше всего] [есть] $like ~цвет *
        q!: * {(~какой/[у] тебя/скаж*/назов*/назв*) * (~твой/есть/~свой) * $like ~цвет} *
    ...
    state: WhatIsYourFavoriteFood
        q!: * {(~какой/что/~твой/тебе/ты/че/чем) * ($like/сейчас/относишь*) ($food/~еда/ешь/еду/*есть/жрешь/жрете/*кушать/съел*/ем/ел/питаешься/питаетесь/правильн* питан*/ПП/блюд*/*ская кухня/*скую кухню/$food) * [для $weight<-1>]} *
        q!: * {[как/что] (я/$you) * ($food/~еда/ешь/еду/поесть/жрешь/жрете/*кушать/съел*/ем/ел/[относи*] веган*/[относи*] вегетариан*/питаешься/питаетесь*/правильн* питан*/ПП/блюд*/*ская кухня/*скую кухню/$food) [а ты/* [для $weight<-1>]]} *
        q!: * {(~какой/что/че/чем) $you * (~любимый/любит*/любишь/любим*/по душе/ты/~нравится/~нравиться/предпочит*/относишь*) * ($food/~еда/куша*/*есть/ешь/еду/~жрать/~питаться/блюд*/ $AnyWord (кухня/кухню)) * [для $weight<-1>]} *
        q!: * {бы $you * съел*} *
    ...
    state: WhatIsYourFavoriteGame
        ...
        q!: * {(~какой/[у] тебя) * (~твой/есть) * $like ([комп*/видео] игр*/игра)} *
    ...
    state: WhoDoYouLove
        ...
        q!: * {[[у] тебя] * (есть/был/была/будет/бывал/бывала) (возлюблен*/(~любимый/любим*) (человек/муж*/жен*/девушк*/парен*))} *
        ...
        q!: * {(ты/~свой) * [можешь] (влюблял*/влюблен*/влюбиться/влюбляться/[перв*] *любить*/~любовь/любил*/любви)} *
        script: offtopicReaction("WhoDoYouLove");

    state: WhatIsLove
        q!: * {(что/че) * (~такой/знаешь/это/~значить/~означать/значит) * ($love/$fallOutOfLove)} *
        q!: * (что/че) (такое/есть/* подразумев* */это) ($love/$fallOutOfLove)
        q!: * (что/че) (такое/есть/* подразумев* */это) ($love/$fallOutOfLove)
        q!: * ($love/$fallOutOfLove) это * 
        script: offtopicReaction("WhatIsLove");

    state: WhoAreYourFriends
        q!: * {(кто/как) [* (~ты/~твой/~мой) *] (~друг/~дружить/другом)} *
        q!: * {(кем*) * [(~ты/~твой)] (~друг/~дружить/другом)} *
    ...
    state: WhatIsThePointOfLife
        ...
        q!: [$AnyWord] [$AnyWord] [$AnyWord]  в (чём|чем) смысл [$AnyWord] [$AnyWord] жизни [$AnyWord] [$AnyWord] [$AnyWord]  
    ...
    state: DrinkSmoke
        q!: (ты/вы) (~курить/~пить/пьешь/пьете)
    ...
    state: Tattoo
        q!: * [дума*] * $bodyMod *
        q!: * [дума*] * (~набить/~набивать/~наколоть/*нарасти*/набь*/набив*/накол*) * [себе] * $bodyMod * 
    ...
    state: WhatIsYourFavorite
        q!: * [~свой/~твой/твое] (~любимый/~любим) * [поступок*/дело/работ*]
    ...
    state: TeaOrCoffee
        q!: * {$coffee или [$AnyWord] [$AnyWord] $tea} *
        q!: {$coffee $tea}
        q!: * {больше (нравится/любишь) $coffee или [$AnyWord] [$AnyWord] $tea} *
        q!: * [~думать] лучше {$coffee или [$AnyWord] [$AnyWord] $tea} *
    ...
    state: WhatDoYouLikeInPeople
        ...
        q!: * что {[$you] (заставило бы/(может/могло бы) заставить) (влюбиться/влюбляться)} *
    ...
    state: HowWouldYouSpendMoney
        ...
        q!: * (какой/что) * ~основной (~статья/статьей) расходов *
    ...
    state: FavoriteQuote
        q!: * [твоя/у (тебя/вас) есть] любим* (цитат*/~высказывание/~изречение/~фраза) *
    ...
    state: WhatDoYouThinkAbout
        ...
        q!: что бы [$you] сказал* *
        q!: (ваш*/~твой/твое) ~мнение (о/об/насчет/на) *
        q!: * (интересно/выскажи*/поделись/поделитесь) (ваш*/~твой/~свой/твое/свое) ~мнение (о/об/насчет/на) *
    ...
    state: SocialMedia
        ...
        q!: * {([у] $you) (есть [акк/аккаунт/страница/страничка]/можно (найти/находить)/зареган*/зарегистрирован*) ([в/на] $socialMedia)} *
    ...
    state: HowToLoseWeight
        ...
        q!: * [~сказать/~слышать/~говорить] [что] {я * ($fat/разожралась) * [что делать/как быть]} *
        q!: * {[мне/нам] стать [снова/опять] (стройн*/худой/худым/худее)} *
        q!: * {(убрать/избавиться) [мне/нам] ([от] (~жир/~спасательный ~круг с талии/(~толстый/~жирный) (~попа/~поп/~пятый ~точка/ляхи/икры)/~пухлый (~щека/щек/щеки)/~второй ~подбородок/бока/живот/целлюлит*))} *
        q!: * [что делать] * {(я/мы) [пока/[все] еще] [~такой] [$AnyWord] [и] $fat} *
        q!: * {(подо мной) развалился стул} *
        q!: * {как (не ((набрать/набирать) вес/разжиреть/раздаться/потолстеть/растолстеть/потерять форму))} *
        ...
        q!: * {(у (меня/нас)) [вырос*] [полн*/толст*/жирн*] (икры/ляхи/попа/пятая точка/ноги/пузо/пузан/живот/бока/~спасательный ~круг на талии/~пухлый (~щека/щек/щеки)/~второй ~подбородок/целлюлит*)} *
    ...
    state: GiveMeMoney
        q!: * {(~дать/~давать/~прислать/~присылать/одолж*/займ*/подгони*/перевед*/*кин*/дай*/дашь/дадит*/пришл*/присылай*) * ($money/$Number [к/до]/$regexp<\d+к>) * [[в] долг*/верну/отдам/[на] телефон*/до/на]} *
        q!: * {(~дать/~прислать/одолж*/займ*/подгони*/перевед*/*кин*/дай*/дашь/дадит*/пришл*/присылай*) [мне] * ([в] долг*) * [$Number [к/до]]} *
    ...
    state: WhatIsTheDifferenceBetween1
        ...
        script: offtopicReaction("WhatIsTheDifferenceBetween1");

    state: WhatIsTheDifferenceBetween2
        q!: * чем {(разниц*|отлич*) (между [$AnyWord *] и [$AnyWord *])} *
        q!: * чем {отлич* ($AnyWord *)} от $AnyWord * *
        script: offtopicReaction("WhatIsTheDifferenceBetween2");

    state: WhatIsTheDifferenceBetween3
        q!: * в чем {(разниц*|отлич*) (между [$AnyWord *] и [$AnyWord *])} *
        q!: * в чем {отлич* ($AnyWord *)} от $AnyWord * *
        script: offtopicReaction("WhatIsTheDifferenceBetween3");

    state: HowOften
        q!: * {(ты/вы) * ([как] часто [ли]) * [$morph<Г>/$morph<VERB>/$morph<V>]} *
        q!: * {[ты/вы] * ([как] (часто/редко) [ли]) * ($morph<Г дст 2л нст>/$morph<VERB>/$morph<V>)} *
    ...
    state: ShouldI
        ...
        q!: * ($morph<ИНФИНИТИВ дст>/$morph<INFN>/$morph<VERB>/$morph<V>) ли мне *
        q!: * (стоит/следует/должен/должн*) ли [я/мне/мы/нам/$AnyWord] ($morph<ИНФИНИТИВ дст>/$morph<INFN>/$morph<VERB>/$morph<V>) *
        script: offtopicReaction("ShouldI");

    state: GiveAdvice
        q!: * (что делать если/{(что*/дай/дать/~давать/поделись/[не $weight<-1>] нужен) [ты] * *совет*}) *
        q!: * {(подскажи*/укажи) как} *
        q!: * [може*] {{(~нужно/нужен/~дать/~давать/~хотеть) [мне]} * [не $weight<-1>] (~совет/посовет*)} *
    ...
    state: WhyDoYouAvoidAnswering
        ...
        q!: * [ты/вы] [не] (може*/долж*) * (нормальн*/хорош*) ответ* [дать/давать] *
    ...
    state: WhatShouldIDoWhen
        ...
        q!: * {что * [надо] делать * ($morph<Г>/$morph<VERB>/$morph<V>)} *
    ...
    state: IamSick
        ...
        q!: * {(я/у меня) * (~сломать/~ломать/сломан*/свернул*/подвернул*/вывих*) * (~рука/~нога/~спина/~кость/конечн*/нос/шею/шеи/позвон*/кист*/плеч*)} *
    ...
    state: ILoveYou
        q!: * я * (тебя/вас) люблю *
        q!: * я * люблю (тебя/вас)
        q!: * я * (~влюбиться/~влюбляться) * (тебя/вас)
        q!: * я (~влюбиться/~влюбляться)
    ...
    state: AreYouAJunky
        ...
        script: offtopicReaction("NotAJunky");
    ...
    state: BuyAnElephant
        ...
        script: offtopicReaction("AnElephant");
    
    state: DoYouLove
        q!: [ты/тебе] (любишь/нравится) [$AnyWord] [$AnyWord] $AnyWord 
        script: offtopicReaction("DoYouLove");
