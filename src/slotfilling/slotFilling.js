bind("preProcess", function (ctx) {
    ...
    ctx.response.replies = [{
        ...
        parameters: {
            ...
            nerClientId: ctx.client["nerClientId"],
            zRefTime: ctx.client["zRefTime"],
            channelUserId: ctx.request.channelUserId
        }
    }];
...
function returnFromSlotFilling(ctx) {
    ...
    // изменить nlp class для отображения в аналитике в диалогах
    // без этого там отображается /SlotFilling/return что не корректно
    // проставляем стейт из сценария в котором мы были до того как попали в слотфилинг
    // так же проставляем текущей intent
    ctx.logInfo = ctx.logInfo || {}
    ctx.logInfo.nlpClass = ctx.session.slotFillingTargetState
    ctx.logInfo.ruleType = "intent"
    ctx.logInfo.rule = (ctx.request.data.intent || {}).path
}
